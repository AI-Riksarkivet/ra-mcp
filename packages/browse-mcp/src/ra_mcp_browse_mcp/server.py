"""Standalone server for ra-browse-mcp.

Run this module directly to start the browse MCP server in isolation,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Riksarkivet Browse MCP Server")
    ...
    if args.stdio:
        browse_mcp.run(transport="stdio")
    else:
        logger.info("MCP Server listening on http://localhost:%d/mcp", args.port)
        browse_mcp.run(transport="streamable-http", host="0.0.0.0", port=args.port, path="/mcp")
