...
async def test_get_json_success(respx_mock):
    ...
    respx_mock.get("https://api.example.com/data").mock(return_value=httpx.Response(200, json=payload))
...
async def test_get_json_with_params(respx_mock):
    respx_mock.get("https://api.example.com/search").mock(return_value=httpx.Response(200, json={"q": "test"}))

    client = HTTPClient()
    try:
        result = await client.get_json("https://api.example.com/search", params={"q": "test", "limit": 10})
    finally:
        await client.aclose()
...
async def test_get_json_with_custom_headers(respx_mock):
    ...
    client = HTTPClient()
    try:
        await client.get_json("https://api.example.com/data", headers={"X-Custom": "value"})
    finally:
        await client.aclose()
...
async def test_get_json_timeout_raises(respx_mock):
    respx_mock.get("https://api.example.com/slow").mock(side_effect=httpx.ReadTimeout("read timed out"))
...
async def test_get_json_connect_error_raises(respx_mock):
    respx_mock.get("https://api.example.com/down").mock(side_effect=httpx.ConnectError("connection refused"))
...
async def test_get_json_invalid_json_raises(respx_mock):
    respx_mock.get("https://api.example.com/bad").mock(return_value=httpx.Response(200, content=b"not json"))
...
async def test_get_json_non_200_raises(respx_mock):
    respx_mock.get("https://api.example.com/err").mock(return_value=httpx.Response(403, text="Forbidden"))
...
async def test_get_json_timeout_env_override(respx_mock, monkeypatch):
    ...
    respx_mock.get("https://api.example.com/data").mock(return_value=httpx.Response(200, json={"ok": True}))
...
async def test_get_xml_success(respx_mock):
    ...
    respx_mock.get("https://api.example.com/xml").mock(return_value=httpx.Response(200, content=xml_body))
...
async def test_get_xml_with_params_and_headers(respx_mock):
    respx_mock.get("https://api.example.com/xml").mock(return_value=httpx.Response(200, content=b"<ok/>"))
...
async def test_get_xml_timeout_raises(respx_mock):
    respx_mock.get("https://api.example.com/xml").mock(side_effect=httpx.ReadTimeout("read timed out"))
...
async def test_get_xml_connect_error_raises(respx_mock):
    respx_mock.get("https://api.example.com/xml").mock(side_effect=httpx.ConnectError("refused"))
...
async def test_get_xml_non_200_raises(respx_mock):
    respx_mock.get("https://api.example.com/xml").mock(return_value=httpx.Response(500, text="fail"))
...
async def test_get_content_success(respx_mock):
    body = b"\x89PNG\r\n\x1a\nbinary data"
    respx_mock.get("https://api.example.com/image").mock(return_value=httpx.Response(200, content=body))
...
async def test_get_content_returns_none_on_404(respx_mock):
    respx_mock.get("https://api.example.com/missing").mock(return_value=httpx.Response(404))
...
async def test_get_content_returns_none_on_non_200(respx_mock):
    respx_mock.get("https://api.example.com/err").mock(return_value=httpx.Response(403))
...
async def test_get_content_returns_none_on_timeout(respx_mock):
    respx_mock.get("https://api.example.com/slow").mock(side_effect=httpx.ReadTimeout("timed out"))
...
async def test_get_content_returns_none_on_connect_error(respx_mock):
    respx_mock.get("https://api.example.com/down").mock(side_effect=httpx.ConnectError("refused"))
...
async def test_get_content_with_headers(respx_mock):
    respx_mock.get("https://api.example.com/image").mock(return_value=httpx.Response(200, content=b"OK"))

    client = HTTPClient()
    try:
        await client.get_content("https://api.example.com/image", headers={"Authorization": "Bearer token"})
    finally:
        await client.aclose()
...
async def test_retry_exhausted_raises(respx_mock):
    respx_mock.get("https://api.example.com/down").mock(return_value=httpx.Response(503))
...
async def test_no_retry_on_non_retryable_status(respx_mock):
    respx_mock.get("https://api.example.com/bad").mock(return_value=httpx.Response(400, text="Bad Request"))
...
def test_retryable_status_codes_set():
    assert {429, 500, 502, 503, 504} == _RETRYABLE_STATUS_CODES
...
def test_get_http_client_enable_logging(tmp_path, monkeypatch):
    ...
    assert any(isinstance(h, logging.FileHandler) and h.baseFilename.endswith("ra_mcp_api.log") for h in root.handlers)

    # Second call should not duplicate the handler
    count_before = sum(1 for h in root.handlers if isinstance(h, logging.FileHandler) and h.baseFilename.endswith("ra_mcp_api.log"))
    get_http_client(enable_logging=True)
    count_after = sum(1 for h in root.handlers if isinstance(h, logging.FileHandler) and h.baseFilename.endswith("ra_mcp_api.log"))
    assert count_after == count_before

    # Cleanup: remove the file handler so it doesn't leak into other tests
    root.handlers = [h for h in root.handlers if not (isinstance(h, logging.FileHandler) and h.baseFilename.endswith("ra_mcp_api.log"))]
...
async def test_get_content_returns_none_on_generic_exception(respx_mock):
    ...
    respx_mock.get("https://api.example.com/crash").mock(side_effect=RuntimeError("unexpected"))
