...
from ra_mcp_search_lib.models import PageInfo, Snippet
...
def test_extract_hit_pages_with_snippets(make_search_record):
...
def test_extract_hit_pages_no_transcribed_text(make_search_record):
...
def test_extract_hit_pages_empty_snippets(make_search_record):
    record = make_search_record(snippets=[], num_total=0)
    screen = DocumentScreen(record=record, keyword="test")
    assert screen._extract_hit_pages() == set()


def test_extract_hit_pages_compound_page_ids(make_search_record):
    record = make_search_record(
        snippets=[
