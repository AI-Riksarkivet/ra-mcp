...
async def test_page_viewer_set_page_text(make_page_context):
...
async def test_page_viewer_empty_page(make_page_context):
    async with PageViewerApp().run_test() as pilot:
        viewer = pilot.app.query_one(PageViewer)
...
async def test_page_viewer_set_page_header(make_page_context):
    async with PageViewerApp().run_test() as pilot:
        viewer = pilot.app.query_one(PageViewer)
...
async def test_page_viewer_set_page_shows_text_area(make_page_context):
    async with PageViewerApp().run_test() as pilot:
        viewer = pilot.app.query_one(PageViewer)
        viewer.show_loading("Loading...")
        await pilot.pause()
...
async def test_page_viewer_links_with_all_urls(make_page_context):
    async with PageViewerApp().run_test() as pilot:
        viewer = pilot.app.query_one(PageViewer)
...
async def test_page_viewer_links_no_text(make_page_context):
