...
def test_search_transcribed_delegates_to_operations(make_search_result):
...
def test_search_metadata_delegates_to_operations(make_search_result):
    service = ArchiveService()
...
def test_browse_document_delegates_to_operations(make_browse_result):
    service = ArchiveService()
...
def test_search_transcribed_default_params(make_search_result):
    service = ArchiveService()
...
def test_browse_document_default_params(make_browse_result):
    service = ArchiveService()
    expected = make_browse_result()
    with patch.object(service._browse, "browse_document", new_callable=AsyncMock, return_value=expected) as mock:
...
def test_service_reuses_event_loop(make_search_result):
