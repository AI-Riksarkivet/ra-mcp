...
def test_node_metadata_depth_0_returns_none(make_search_record):
    record = make_search_record()
    assert ResultList._node_metadata(record, 0) is None


def test_node_metadata_depth_1_archival_institution(make_search_record):
    record = make_search_record(
...
def test_node_metadata_depth_2_hierarchy(make_search_record):
    record = make_search_record(
...
def test_node_metadata_depth_2_fallback_provenance(make_search_record):
    record = make_search_record(
        hierarchy=None,
...
def test_node_metadata_depth_3_hierarchy_index(make_search_record):
    record = make_search_record(
        hierarchy=[
...
def test_node_metadata_depth_beyond_hierarchy_returns_none(make_search_record):
    record = make_search_record(hierarchy=[GenericReference(caption="only-one")])
    assert ResultList._node_metadata(record, 4) is None


def test_node_metadata_depth_1_no_institution_returns_none(make_search_record):
...
def _FakeNode(data: object) -> TreeNode:
    """Minimal stand-in for a TreeNode carrying only ``.data``."""
    return cast(TreeNode, SimpleNamespace(data=data))


def test_ref_from_node_search_record(make_search_record):
    record = make_search_record(reference_code="SE/RA/123")
    assert ResultList._ref_from_node(_FakeNode(record)) == "SE/RA/123"


def test_ref_from_node_snippet_node(make_search_record):
...
def test_link_from_node_search_record(make_search_record):
...
def test_link_from_node_search_record_no_link(make_search_record):
    record = make_search_record(html_link=None)
    assert ResultList._link_from_node(_FakeNode(record)) is None


def test_link_from_node_snippet_node(make_search_record):
