"""Standalone server for ra-mcp-viewer-mcp.

Run this module directly to start the viewer MCP server in isolation,
...
def main() -> None:
    ...
    if args.stdio:
        viewer_mcp.run(transport="stdio")
    else:
        logger.info("MCP Server listening on http://localhost:%d/mcp", args.port)
        viewer_mcp.run(transport="streamable-http", host="0.0.0.0", port=args.port, path="/mcp")
