"""In-memory ``Client`` smoke test for the aktiebolag-mcp MCP server.
...
async def test_aktiebolag_mcp_registers_expected_tools():
    async with Client(aktiebolag_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_bolag" in names


async def test_aktiebolag_mcp_tools_are_well_formed():
    async with Client(aktiebolag_mcp) as client:
        tools = await client.list_tools()
