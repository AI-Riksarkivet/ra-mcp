"""In-memory ``Client`` smoke test for the diplomatics-mcp MCP server.
...
async def test_diplomatics_mcp_registers_expected_tools():
    async with Client(diplomatics_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_sdhk" in names


async def test_diplomatics_mcp_tools_are_well_formed():
    async with Client(diplomatics_mcp) as client:
        tools = await client.list_tools()
