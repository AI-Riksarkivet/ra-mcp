"""In-memory ``Client`` smoke test for the rosenberg-mcp MCP server.
...
async def test_rosenberg_mcp_registers_expected_tools():
    async with Client(rosenberg_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_rosenberg" in names


async def test_rosenberg_mcp_tools_are_well_formed():
    async with Client(rosenberg_mcp) as client:
        tools = await client.list_tools()
