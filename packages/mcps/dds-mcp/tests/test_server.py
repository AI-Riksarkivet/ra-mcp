"""In-memory ``Client`` smoke test for the dds-mcp MCP server.
...
async def test_dds_mcp_registers_expected_tools():
    async with Client(dds_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_doda" in names


async def test_dds_mcp_tools_are_well_formed():
    async with Client(dds_mcp) as client:
        tools = await client.list_tools()
