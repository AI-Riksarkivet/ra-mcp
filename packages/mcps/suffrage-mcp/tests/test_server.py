"""In-memory ``Client`` smoke test for the suffrage-mcp MCP server.
...
async def test_suffrage_mcp_registers_expected_tools():
    async with Client(suffrage_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_fkpr" in names


async def test_suffrage_mcp_tools_are_well_formed():
    async with Client(suffrage_mcp) as client:
        tools = await client.list_tools()
