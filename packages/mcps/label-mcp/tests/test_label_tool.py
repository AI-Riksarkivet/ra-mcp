...
async def test_tool_missing_ls_url():
    ...
    with patch.dict("os.environ", env, clear=True), pytest.raises(ToolError, match=r"(?i)url required"):
        async with Client(label_mcp) as client:
            await client.call_tool(
                "import_to_label_studio",
                {"image_urls": ["https://img.test/1.jpg"]},
...
async def test_tool_missing_token():
    ...
    with patch.dict("os.environ", env, clear=True), pytest.raises(ToolError, match=r"(?i)token required"):
        async with Client(label_mcp) as client:
            await client.call_tool(
                "import_to_label_studio",
                {"image_urls": ["https://img.test/1.jpg"]},
...
async def test_tool_missing_project_id():
    env = {k: v for k, v in os.environ.items() if k != "LS_PROJECT_ID"}
    env["LS_URL"] = "https://ls.test"
    env["LS_TOKEN"] = "abc"  # noqa: S105
    with patch.dict("os.environ", env, clear=True), pytest.raises(ToolError, match=r"(?i)project id required"):
        async with Client(label_mcp) as client:
            await client.call_tool(
                "import_to_label_studio",
                {"image_urls": ["https://img.test/1.jpg"]},
...
async def test_tool_import_api_error():
    with (
        ...
        pytest.raises(ToolError, match="API error 500"),
    ):
        async with Client(label_mcp) as client:
            await client.call_tool(
                "import_to_label_studio",
