...
def test_import_tasks_api_error():
    ...
    with patch("ra_mcp_label_mcp.ls_client._get_client", return_value=mock_client), pytest.raises(RuntimeError, match="Label Studio API error 403"):
...
def test_import_tasks_generic_error():
    ...
    with patch("ra_mcp_label_mcp.ls_client._get_client", return_value=mock_client), pytest.raises(RuntimeError, match="Label Studio API request failed"):
        import_tasks([{"data": {"ocr": "a"}}], "https://ls.example.com", "token", 1)
...
def test_assign_tasks_user_not_found():
    ...
    with patch("ra_mcp_label_mcp.ls_client._get_client", return_value=mock_client), pytest.raises(RuntimeError, match="No Label Studio user found"):
...
def test_assign_tasks_api_error():
    ...
    with patch("ra_mcp_label_mcp.ls_client._get_client", return_value=mock_client), pytest.raises(RuntimeError, match="Label Studio assignment error 500"):
