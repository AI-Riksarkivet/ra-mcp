"""In-memory ``Client`` smoke test for the tora-mcp MCP server.
...
async def test_tora_mcp_registers_expected_tools():
    async with Client(tora_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_tora" in names


async def test_tora_mcp_tools_are_well_formed():
    async with Client(tora_mcp) as client:
        tools = await client.list_tools()
