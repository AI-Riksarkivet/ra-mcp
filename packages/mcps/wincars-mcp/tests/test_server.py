"""In-memory ``Client`` smoke test for the wincars-mcp MCP server.
...
async def test_wincars_mcp_registers_expected_tools():
    async with Client(wincars_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_wincars" in names


async def test_wincars_mcp_tools_are_well_formed():
    async with Client(wincars_mcp) as client:
        tools = await client.list_tools()
