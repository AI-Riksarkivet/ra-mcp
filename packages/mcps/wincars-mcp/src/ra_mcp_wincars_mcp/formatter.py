...
from ra_mcp_wincars_lib.models import TYP_DISPLAY
...
def _format_wincars_record(rec: dict[str, Any], lines: list[str]) -> None:
    ...
    typ_name = TYP_DISPLAY.get(typ_code, typ_code)
    if typ_code:
        lines.append(f"Type: {typ_name} ({typ_code})")

    append_if(lines, "Make", rec.get("fabrikat", ""))
    ...
    append_if(lines, "Chassis", rec.get("cnr", ""))
    append_if(lines, "Engine", rec.get("mnr", ""))
    append_if(lines, "Domicile", rec.get("hemvist", ""))
    append_if(lines, "Registered", rec.get("antag", ""))
    append_if(lines, "Deregistered", rec.get("avreg", ""))
    ...
    append_if(lines, "Notes", rec.get("anm", ""))
    append_if(lines, "Archive", rec.get("arkivkod", ""))
    append_if(lines, "Volume", rec.get("volym", ""))
