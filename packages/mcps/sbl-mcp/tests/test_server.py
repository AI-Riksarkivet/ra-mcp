"""In-memory ``Client`` smoke test for the sbl-mcp MCP server.
...
async def test_sbl_mcp_registers_expected_tools():
    async with Client(sbl_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_sbl" in names


async def test_sbl_mcp_tools_are_well_formed():
    async with Client(sbl_mcp) as client:
        tools = await client.list_tools()
