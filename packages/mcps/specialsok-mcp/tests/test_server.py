"""In-memory ``Client`` smoke test for the specialsok-mcp MCP server.
...
async def test_specialsok_mcp_registers_expected_tools():
    async with Client(specialsok_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_fangrullor" in names


async def test_specialsok_mcp_tools_are_well_formed():
    async with Client(specialsok_mcp) as client:
        tools = await client.list_tools()
