"""In-memory ``Client`` smoke test for the faltjagare-mcp MCP server.
...
async def test_faltjagare_mcp_registers_expected_tools():
    async with Client(faltjagare_mcp) as client:
        tools = await client.list_tools()

    names = {t.name for t in tools}
    assert "search_faltjagare" in names


async def test_faltjagare_mcp_tools_are_well_formed():
    async with Client(faltjagare_mcp) as client:
        tools = await client.list_tools()
