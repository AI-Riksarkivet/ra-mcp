...
class SearchScreen(Screen):
    ...
    def on_mount(self) -> None:
        if self._initial_keyword:
            ...
            self._run_search(self._initial_keyword, bar.mode, limit=bar.limit)
        else:
            self.query_one(SearchBar).focus_input()

    def on_search_bar_submitted(self, event: SearchBar.Submitted) -> None:
        self._current_offset = 0
        self._run_search(event.keyword, event.mode, limit=event.limit)

    def _run_search(self, keyword: str, mode: str, offset: int = 0, limit: int | None = None) -> None:
        ...
        page_limit = self._current_limit
        if mode == "transcribed":
            fn = lambda: service.search_transcribed(keyword, offset=offset, limit=page_limit)  # noqa: E731
        else:
            fn = lambda: service.search_metadata(keyword, offset=offset, limit=page_limit)  # noqa: E731
            ...
            if result.contexts and record:
                self.app.push_screen(
                    PageScreen(
                        ...
                        reference_code=record.metadata.reference_code or "",
                    )
                )
            else:
    ...
    def on_result_list_snippet_selected(self, event: ResultList.SnippetSelected) -> None:
        ...
        service = self._service.service
        ref_code = event.record.metadata.reference_code or ""
    ...
    def action_next_page(self) -> None:
        next_offset = self._current_offset + self._current_limit
    ...
    def action_prev_page(self) -> None:
        if self._current_keyword and self._current_offset > 0:
            prev_offset = max(0, self._current_offset - self._current_limit)
