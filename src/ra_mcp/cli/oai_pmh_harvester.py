...
class OAIPMHClient:
    ...
    def __init__(self, base_url: str = OAI_BASE_URL):
        ...
        self.session.headers.update({
            'User-Agent': 'Riksarkivet OAI-PMH Harvester/1.0'
        })
    ...
    def list_sets(self) -> List[Dict[str, str]]:
        ...
        return [
            {'spec': 'SE_RA', 'name': 'Riksarkivet (National Archives)', 'description': 'Medieval parchments, government records'},
            {'spec': 'SE_KrA', 'name': 'Krigsarkivet (Military Archives)', 'description': 'Military records, war documentation'},
            {'spec': 'SE_ULA', 'name': 'Uppsala landsarkiv', 'description': 'Church records, parish registers'},
            {'spec': 'SE_GLA', 'name': 'Göteborg landsarkiv', 'description': 'Western Sweden regional archives'},
            {'spec': 'SE_HLA', 'name': 'Härnösand landsarkiv', 'description': 'Northern Sweden regional archives'},
            {'spec': 'SE_LLA', 'name': 'Lund landsarkiv', 'description': 'Southern Sweden regional archives'},
            {'spec': 'SE_VALA', 'name': 'Vadstena landsarkiv', 'description': 'Central Sweden regional archives'},
            {'spec': 'SE_ViLA', 'name': 'Visby landsarkiv', 'description': 'Gotland regional archives'},
            {'spec': 'SE_ÖLA', 'name': 'Östersund landsarkiv', 'description': 'Northern inland regional archives'},
        ]
    ...
    def list_identifiers(self, set_spec: Optional[str] = None, 
    ...
    def harvest_records(self, from_date: Optional[str] = None,
    ...
    def get_recent_records(self, set_spec: str, days_back: int = 10,
                          metadata_prefix: str = 'oai_ape_ead',
                          limit: Optional[int] = None) -> List[Dict[str, Any]]:
...
def list_identifiers(client, set_spec, metadata_prefix, from_date, until_date, limit):
    ...
    try:
        ...
        # Create table
        table = Table(title=f"Record Identifiers" + (f" from {set_spec}" if set_spec else ""))
