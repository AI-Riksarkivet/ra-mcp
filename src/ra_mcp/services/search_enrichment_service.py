...
class SearchEnrichmentService:
        ...
        for pid, pid_hits in hits_by_pid.items():
            ...
            if collection_info and collection_info.get("manifests"):
                ...
                for hit in pid_hits:
                    ...
                    page_ref = (
                        f"{hit.reference_code or hit.pid[-8:]}:p{hit.page_number}"
                    )
                    self._enrich_single_hit(hit, manifest_id, search_term)

                    # Try to get ALTO content
                    if hit.alto_url:
                        full_text = self.alto_client.fetch_content(
                            hit.alto_url, timeout=8
                        )
                        if full_text:
    ...
    def _enrich_single_hit(
        self, hit: SearchHit, manifest_id: str, search_term: Optional[str]
    ):
        ...
        hit.bildvisning_url = URLGenerator.bildvisning_url(
            manifest_id, hit.page_number, search_term
        )

    def expand_hits_with_context_padding(
        self, hits: List[SearchHit], padding: int = 1
    ) -> List[SearchHit]:
        ...
        for (pid, ref_code), doc_hits in hits_by_doc.items():
            ...
            for hit in doc_hits:
                try:
                    page_num = (
                        int(hit.page_number.lstrip("0"))
                        if hit.page_number.lstrip("0").isdigit()
                        else int(hit.page_number)
                    )
    ...
    def _find_existing_hit(
        self, doc_hits: List[SearchHit], page
    ) -> Optional[SearchHit]:
        """Find existing hit for a given page."""
        for hit in doc_hits:
            hit_page_num = (
                hit.page_number.lstrip("0")
                if hit.page_number.lstrip("0").isdigit()
    ...
    def _create_context_hit(
        self, pid: str, ref_code: str, page, doc_hits: List[SearchHit]
    ) -> SearchHit:
        ...
        return SearchHit(
            ...
            archival_institution=template_hit.archival_institution
            if template_hit
            else None,
